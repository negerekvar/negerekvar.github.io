...
p5.prototype.collideDebug = function(debugMode) { _collideDebug = debugMode };
...
p5.prototype.collideLineLine = function(x1, y1, x2, y2, x3, y3, x4, y4, calcIntersection) {
    ...
    if (uA >= 0 && uA <= 1 && uB >= 0 && uB <= 1) {
        ...
        if (calcIntersection) { intersection = { x: intersectionX, y: intersectionY };
            return intersection } else {
            return true } }
    if (calcIntersection) { intersection = { x: false, y: false };
        return intersection }
    return false };
p5.prototype.collideLineRect = function(x1, y1, x2, y2, rx, ry, rw, rh, calcIntersection) {
    var left, right, top, bottom, intersection;
    if (calcIntersection) { left = this.collideLineLine(x1, y1, x2, y2, rx, ry, rx, ry + rh, true);
        ...
        intersection = { left: left, right: right, top: top, bottom: bottom } } else { left = this.collideLineLine(x1, y1, x2, y2, rx, ry, rx, ry + rh);
...
p5.prototype.collidePointPoly = function(px, py, vertices) {
    var collision = false;
    var next = 0;
    for (var current = 0; current < vertices.length; current++) { next = current + 1;
...
p5.prototype.collidePolyPoly = function(p1, p2, interior) {
    if (interior == undefined) { interior = false }
    var next = 0;
    for (var current = 0; current < p1.length; current++) { next = current + 1;
        if (next == p1.length) next = 0;
        var vc = p1[current];
        var vn = p1[next];
        var collision = this.collideLinePoly(vc.x, vc.y, vn.x, vn.y, p2);
        if (collision) return true;
        if (interior == true) { collision = this.collidePointPoly(p2[0].x, p2[0].y, p1);
            if (collision) return true } }
    return false };
